...
masked2022<-read_xlsx("2022_phenos_and_maskedNDVI.xlsx")
#NA not permitted in predictors, so drop a few column, which contains NA and others 
masked2022<- select(masked2022, -1,-2, -5, -6, -9, -10, -13, -14, -15, -18, -21, -24, -27, -30, -33, -36, -39, -42, -45, -48 )
#a few rows contains NA value, if discard we can use code below

summary(masked2022)
masked2022$Anthesis_DAP <- as.numeric(masked2022$Anthesis_DAP)
masked2022$Silking_DAP <- as.numeric(masked2022$Silking_DAP)
masked2022$spadavg <- as.numeric(masked2022$spadavg)
#seperating training and testing data set
set.seed(123)
ind <- sample(2, nrow(masked2022), replace = TRUE, prob = c(0.75, 0.25))
train <- masked2022[ind==1,]
test <- masked2022[ind==2,]
# 604 data points for training and 203 data points for testing
...
# "RMSE: 33.9304624656835"

masked2022.sum.plot<- ggplot(data = test.df, aes(x = test.y, y = y_predicted)) +
  geom_point() + theme_bw() +
  geom_smooth(method = 'lm', col = 'blue') +
  labs(title = "Masked 2022 Actual vs Predicted Yields", x = "Actual Yield", y = "Predicted Yield")
masked2022.sum.plot + geom_label(label="RMSE = 33.9305", x=75,  y=175, label.size = 0.35, color = "red")+ylim(50, 200)

####################for mean masked 2022
y <- data.matrix(train$Yield)
...
test.df <- as.data.frame(test.df)
rmse <- sqrt(mean((y_predicted - test.y)^2))
print(paste("RMSE:", rmse)) # RMSE: 33.8796633245868

masked2022.mean.plot<-ggplot(data = test.df, aes(x = test.y, y = y_predicted)) +
  geom_point() + theme_bw() +
  geom_smooth(method = 'lm', col = 'blue') +
  labs(title = "Masked 2022 Actual vs Predicted Yields", x = "Actual Yield", y = "Predicted Yield")

masked2022.mean.plot + geom_label(label="RMSE = 33.8797", x=75,  y=180, label.size = 0.35, color = "red")+ylim(50,200)
